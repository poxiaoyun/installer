...
func UpgradeStrategy(obj metav1.Object) (string, error) {
	value := obj.GetAnnotations()[apps.AnnotationUpgradeStrategy]
	switch value {
	case "", UpgradeStrategyRetain, UpgradeStrategyRecreate:
		return value, nil
	default:
		return "", fmt.Errorf("invalid %s %q on %s/%s: expected %q or %q",
			apps.AnnotationUpgradeStrategy, value, obj.GetNamespace(), obj.GetName(),
...
func RemoveStrategy(obj metav1.Object) (string, error) {
	value := obj.GetAnnotations()[apps.AnnotationRemoveStrategy]
	switch value {
	case "", RemoveStrategyRetain:
		return value, nil
	default:
		return "", fmt.Errorf("invalid %s %q on %s/%s: expected %q",
			apps.AnnotationRemoveStrategy, value, obj.GetNamespace(), obj.GetName(),
